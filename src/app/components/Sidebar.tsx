...
export function Sidebar({ active, onNavigate, role, userName, userEmail, userInitials, onLogout }: SidebarProps) {
  const navItems = getNav(role);

  return (
    <aside
      className="flex flex-col h-full"
      style={{ background: "var(--sidebar)", color: "var(--sidebar-foreground)", width: 248, flexShrink: 0 }}
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 py-5 border-b" style={{ borderColor: "var(--sidebar-border)" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: "var(--sidebar-foreground)", opacity: 0.5, letterSpacing: "0.06em", textTransform: "uppercase" }}>
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
        <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--sidebar-foreground)", opacity: 0.4, padding: "4px 8px 6px" }}>
          {role === "estudiante" ? "Portal Estudiantil" : "Módulos"}
        </div>
        {navItems.map((item) => {
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <button
            ...
            className="flex items-center gap-3 rounded-md px-3 py-2.5 w-full text-left transition-all duration-150"
            style={{
              background: active === "perfil" ? "var(--sidebar-primary)" : "transparent",
              color: active === "perfil" ? "#fff" : "var(--sidebar-foreground)",
              fontWeight: active === "perfil" ? 600 : 400,
              fontSize: 13.5,
              cursor: "pointer",
              border: "none",
              outline: "none",
          ...
          >
            <span style={{ opacity: active === "perfil" ? 1 : 0.7 }}><User size={17} /></span>
            Mi Perfil
          </button>
        </div>
      </nav>
      ...
      <div className="px-3 py-4 border-t" style={{ borderColor: "var(--sidebar-border)" }}>
        ...
        <button
          ...
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.opacity = "0.55"; (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}
        >
          <LogOut size={14} />
          Cerrar sesión
        </button>
